...
{
    std::string mode;
    std::string featModeStr = "manual";
    std::string modelPath = "src/sh_transformer.pt";

    int numRounds = 0;
    int numGamesPerRound = 0;
    int numEpochs = 0;
    int numTestingGames = 0;
    int evalGames = 0;
    int mctsIters = 0;
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (a == "--mode" && i + 1 < argc)
            ...
            numRounds = std::stoi(argv[++i]);
        else if (a == "--games-per-round" && i + 1 < argc)
            numGamesPerRound = std::stoi(argv[++i]);
        else if (a == "--epochs" && i + 1 < argc)
            numEpochs = std::stoi(argv[++i]);
        else if (a == "--test-games" && i + 1 < argc)
            numTestingGames = std::stoi(argv[++i]);
        else if (a == "--games" && i + 1 < argc)
            evalGames = std::stoi(argv[++i]);
        else if (a == "--iters" && i + 1 < argc)
            mctsIters = std::stoi(argv[++i]);
        else if (a == "--opponent" && i + 1 < argc)
        {
            while (i + 1 < argc && argv[i + 1][0] != '-')
                opponents.emplace_back(argv[++i]);
        }
        else if (a == "--feat-mode" && i + 1 < argc)
        {
            featModeStr = argv[++i];
        }
    ...
    if (featModeStr == "manual")
        featMode = FeatureMode::ManualOnly;
    else if (featModeStr == "transformer")
        featMode = FeatureMode::TransformerOnly;
    else if (featModeStr == "both")
        featMode = FeatureMode::Both;
    else
    {
        std::cerr << "Unknown feat mode " << featModeStr
                  << " (use manual|transformer|both)\n";
        return 1;
    }
    ...
    if (featMode != FeatureMode::ManualOnly)
    {
        try
        {
            initTransformerModel(modelPath, INPUT_DIM);
        }
        catch (const std::exception &e)
        {
            std::cerr << "Failed to load transformer model from "
                      << modelPath << ": " << e.what() << "\n";
            return 1;
        }
    }
    ...
    else if (mode == "evaluate")
    {
        ...
        for (auto &opp : opponents)
        {
            ...
            if (opp == "random")
                res = play<RandomBot>(evalGames, mctsIters, label.c_str());
            else if (opp == "greedy")
                res = play<GreedyBot>(evalGames, mctsIters, label.c_str());
            else
            {
            ...
            double mctsPct = 100.0 * res.mctsWins / total;
            double oppPct = 100.0 * res.oppWins / total;

            std::cout << label << " (" << mctsIters << " iters): "
                      << "MCTS " << mctsPct << "%, "
                      << opp << " " << oppPct << "%\n";
        }
    }
    ...
    trainingLoggerFlush("training_buffer.bin");
    return 0;
}
